...
*Bot tự động đóng vai PM đi tuần tra dự án mỗi ngày (hiện tại lúc test đang đặt 1 phút/lần). Dữ liệu đầu vào bộ lọc JQL: `Chỉ lấy các Task chưa hoàn thành (Unresolved)`.*
...
*   **Điều kiện kích hoạt:** 
    * Task bị bỏ trống hoặc không có giá trị ở trường `duedate` (Ngày hết hạn) khi được chuyển trạng thái từ `To do` sang `In progress` hoặc `Doing`.
    * **HOẶC** bị bỏ trống khối lượng `timeoriginalestimate` (Estimate tổng lúc grooming).
...
*   **Hành động (Action):** Bot sẽ chọn ngẫu nhiên (random) một câu khen ngợi hài hước, mang tính chất động viên để gửi lên Group. (VD: *"Hôm nay team ngoan quá, xứng đáng 10 điểm không có nhưng! 🚀"*, *"Không một tiếng còi báo động nào, Jira Master xin phép đi ngủ giữ sắc đẹp! 💅"*, *"Cả làng bình yên, các dev nhà mình nỗ lực tuyệt vời quá, mlem mlem! 🍗"*).
